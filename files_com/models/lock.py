...
class Lock:
    ...
    def get_attributes(self):
        return {k: getattr(self, k, None) for k in Lock.default_attributes}
    ...
    def delete(self, params = {}):
        ...
        return Api.send_request("DELETE", "/locks/{path}".format(path=params['path']), params, self.options)
    ...
    def save(self):
        if hasattr(self, "path") and self.path:
            raise NotImplementedError("The Lock object doesn't support updates.")
        else:
            new_obj = Lock.do_create(self.get_attributes(), self.options)
    ...
    def do_list_for(path, params = {}, options = {}):
        ...
        response, options = Api.send_request("GET", "/locks/{path}".format(path=params['path']), params, options)
        return [ Lock(entity_data, options) for entity_data in response.data ]
