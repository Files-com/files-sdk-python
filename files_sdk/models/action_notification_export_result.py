...
class ActionNotificationExportResult:
    ...
    def set_attributes(self, attributes):
        for (attribute, default_value) in ActionNotificationExportResult.default_attributes.items():
            setattr(self, attribute, attributes.get(attribute, default_value))

    def get_attributes(self):
        return {k: getattr(self, k, None) for k in ActionNotificationExportResult.default_attributes if getattr(self, k, None) is not None}
...
def list(params = None, options = None):
    ...
    if "action_notification_export_id" in params and not isinstance(params["action_notification_export_id"], int):
        raise InvalidParameterError("Bad parameter: action_notification_export_id must be an int")
    if "action_notification_export_id" not in params:
        raise MissingParameterError("Parameter missing: action_notification_export_id")
    return ListObj(ActionNotificationExportResult,"GET", "/action_notification_export_results", params, options)
