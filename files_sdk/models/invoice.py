...
class Invoice:
    ...
    def set_attributes(self, attributes):
        for attribute, default_value in Invoice.default_attributes.items():
            ...
            if attribute in Invoice.__decimal_fields and value is not None:
                value = Decimal(str(value))
            if (
                attribute in Invoice.__decimal_array_fields
    ...
    def get_attributes(self):
        ...
        for k in list(attrs.keys()):
            if k in Invoice.__decimal_fields and attrs[k] is not None:
                attrs[k] = str(attrs[k])
            if k in Invoice.__decimal_array_fields and attrs[k] is not None:
...
def find(id, params=None, options=None):
    ...
    response, options = Api.send_request(
        "GET",
        "/invoices/{id}".format(id=quote(str(params["id"]), safe="")),
        params,
        options,
    )
...
def create_export(params=None, options=None):
    ...
    if not isinstance(options, dict):
        options = {}
    response, options = Api.send_request(
        "POST", "/invoices/create_export", params, options
    )
