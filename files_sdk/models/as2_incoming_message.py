...
class As2IncomingMessage:
    default_attributes = {
        ...
        'processing_result': None,     # string - Result of processing. Valid values: `unable_to_find_station`, `unable_to_find_partner`, `unable_to_validate_signature`, `decrypt_fail`, `file_save_fail`, `success`
    ...
    def set_attributes(self, attributes):
        for (attribute, default_value) in As2IncomingMessage.default_attributes.items():
            setattr(self, attribute, attributes.get(attribute, default_value))

    def get_attributes(self):
        return {k: getattr(self, k, None) for k in As2IncomingMessage.default_attributes if getattr(self, k, None) is not None}
...
def list(params = None, options = None):
    ...
    return ListObj(As2IncomingMessage,"GET", "/as2_incoming_messages", params, options)
