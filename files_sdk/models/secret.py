...
class Secret:
    default_attributes = {
        "id": None,  # int64 - Secret ID.
        "workspace_id": None,  # int64 - Workspace ID. 0 means the default workspace.
        "name": None,  # string - Secret name.
        "description": None,  # string - Internal description for your reference.
        "secret_type": None,  # string - Secret type.
        "metadata": None,  # object - Non-secret metadata for the Secret type.
        "value_field_names": None,  # array(string) - Names of configured secret value fields. Secret values are never returned.
        "created_at": None,  # date-time - Secret create date/time.
        "updated_at": None,  # date-time - Secret update date/time.
    }
    ...
    def set_attributes(self, attributes):
        for attribute, default_value in Secret.default_attributes.items():
    ...
    def get_attributes(self):
        attrs = {
            ...
            for k in Secret.default_attributes
    ...
    def update(self, params=None):
        ...
        if "secret_type" in params and not isinstance(
            params["secret_type"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: secret_type must be an str"
            )
        response, _options = Api.send_request(
            "PATCH",
            "/secrets/{id}".format(id=quote(str(params["id"]), safe="")),
            params,
            self.options,
        )
...
def list(params=None, options=None):
    ...
    return ListObj(Secret, "GET", "/secrets", params, options)
...
def find(id, params=None, options=None):
    ...
    return Secret(response.data, options)
...
def create(params=None, options=None):
    ...
    if "secret_type" in params and not isinstance(params["secret_type"], str):
        raise InvalidParameterError(
            "Bad parameter: secret_type must be an str"
        )
    if "metadata" in params and not isinstance(params["metadata"], dict):
        raise InvalidParameterError("Bad parameter: metadata must be an dict")
    ...
    if "secret_type" not in params:
        raise MissingParameterError("Parameter missing: secret_type")
    response, options = Api.send_request("POST", "/secrets", params, options)
    return Secret(response.data, options)
