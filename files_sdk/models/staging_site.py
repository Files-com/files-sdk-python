...
class StagingSite:
    ...
    def set_attributes(self, attributes):
        for attribute, default_value in StagingSite.default_attributes.items():
            value = attributes.get(attribute, default_value)
            if attribute in StagingSite.__decimal_fields and value is not None:
                value = Decimal(str(value))
            if (
                attribute in StagingSite.__decimal_array_fields
    ...
    def get_attributes(self):
        attrs = {
            ...
            for k in StagingSite.default_attributes
            if getattr(self, k, None) is not None
        }
        for k in list(attrs.keys()):
            if k in StagingSite.__decimal_fields and attrs[k] is not None:
                attrs[k] = str(attrs[k])
            if (
                k in StagingSite.__decimal_array_fields
    ...
    def save(self):
        if hasattr(self, "id") and self.id:
            raise NotImplementedError(
                "The StagingSite object doesn't support updates."
            )
        else:
...
def list(params=None, options=None):
    ...
    return ListObj(Site, "GET", "/staging_sites", params, options)
...
def create(params=None, options=None):
    ...
    response, options = Api.send_request(
        "POST", "/staging_sites", params, options
    )
...
def create_export(params=None, options=None):
    ...
    if not isinstance(options, dict):
        options = {}
    response, options = Api.send_request(
        "POST", "/staging_sites/create_export", params, options
    )
    return Export(response.data, options)


def new(*args, **kwargs):
    return StagingSite(*args, **kwargs)
