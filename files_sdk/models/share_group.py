...
class ShareGroup:
    ...
    def get_attributes(self):
        return {k: getattr(self, k, None) for k in ShareGroup.default_attributes if getattr(self, k, None) is not None}
    ...
    def update(self, params = None):
        ...
        if "members" in params and not isinstance(params["members"], builtins.list):
            raise InvalidParameterError("Bad parameter: members must be an list")
        response, _options = Api.send_request("PATCH", "/share_groups/{id}".format(id=params['id']), params, self.options)
    ...
    def delete(self, params = None):
        ...
        if "id" in params and not isinstance(params["id"], int):
            raise InvalidParameterError("Bad parameter: id must be an int")
        response, _options = Api.send_request("DELETE", "/share_groups/{id}".format(id=params['id']), params, self.options)
        return response.data
...
def find(id, params = None, options = None):
    ...
    if "id" not in params:
        raise MissingParameterError("Parameter missing: id")
    response, options = Api.send_request("GET", "/share_groups/{id}".format(id=params['id']), params, options)
...
def create(params = None, options = None):
    ...
    response, options = Api.send_request("POST", "/share_groups", params, options)
    return ShareGroup(response.data, options)
...
def update(id, params = None, options = None):
    ...
    if "id" not in params:
        raise MissingParameterError("Parameter missing: id")
    response, options = Api.send_request("PATCH", "/share_groups/{id}".format(id=params['id']), params, options)
    return ShareGroup(response.data, options)
...
def delete(id, params = None, options = None):
    ...
    if "id" not in params:
        raise MissingParameterError("Parameter missing: id")
    response, _options = Api.send_request("DELETE", "/share_groups/{id}".format(id=params['id']), params, options)
    return response.data
