...
class Site:
    ...
    default_attributes = {
        ...
        "bundle_send_one_time_password_to_recipient_at_registration": None,  # boolean - If true, new Share Links must send a one-time password to the recipient when they register. Requires bundle_require_share_recipient and cannot be enabled with bundle_password_required.
        ...
        "group_admins_can_manage_group_memberships": None,  # boolean - Allow group admins to add or remove existing users in their groups
        ...
        "email_footer_custom_text": None,  # string - Custom footer text for system-generated emails (as Markdown). Supports standard strftime date/time patterns like %Y (4-digit year), %m (month), %d (day).
...
def update(params=None, options=None):
    ...
    if (
        "bundle_send_one_time_password_to_recipient_at_registration" in params
        and not isinstance(
            params[
        ...
        raise InvalidParameterError(
            "Bad parameter: bundle_send_one_time_password_to_recipient_at_registration must be an bool"
        )
    ...
    if (
        "group_admins_can_manage_group_memberships" in params
        and not isinstance(
            params["group_admins_can_manage_group_memberships"], bool
        )
    ):
        raise InvalidParameterError(
            "Bad parameter: group_admins_can_manage_group_memberships must be an bool"
        )
    ...
    if (
        "group_admins_can_bypass_user_lifecycle_rules" in params
        and not isinstance(
            params["group_admins_can_bypass_user_lifecycle_rules"], bool
        )
    ):
        raise InvalidParameterError(
            "Bad parameter: group_admins_can_bypass_user_lifecycle_rules must be an bool"
        )
