...
class RemoteServer:
    ...
    def disconnect_agent(self, params=None):
        if not isinstance(params, dict):
            params = {}
        ...
        if "id" in params and not isinstance(params["id"], int):
            raise InvalidParameterError("Bad parameter: id must be an int")
        if "proxy_url" in params and not isinstance(params["proxy_url"], str):
            raise InvalidParameterError(
                "Bad parameter: proxy_url must be an str"
            )
        if "proxy_host_header" in params and not isinstance(
            params["proxy_host_header"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: proxy_host_header must be an str"
            )
        response, _options = Api.send_request(
            "POST",
            "/remote_servers/{id}/disconnect_agent".format(
                id=quote(str(params["id"]), safe="")
            ),
            params,
            self.options,
    ...
    def authenticate_agent(self, params=None):
        if not isinstance(params, dict):
            params = {}
        ...
        if "id" in params and not isinstance(params["id"], int):
            raise InvalidParameterError("Bad parameter: id must be an int")
        if "signed_nonce" in params and not isinstance(
            params["signed_nonce"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: signed_nonce must be an str"
            )
        if "agent_version" in params and not isinstance(
            params["agent_version"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: agent_version must be an str"
            )
        if "peer_id" in params and not isinstance(params["peer_id"], str):
            raise InvalidParameterError(
                "Bad parameter: peer_id must be an str"
            )
        if "proxy_url" in params and not isinstance(params["proxy_url"], str):
            raise InvalidParameterError(
                "Bad parameter: proxy_url must be an str"
        ...
        if "agent_hostname" in params and not isinstance(
            params["agent_hostname"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: agent_hostname must be an str"
            )
        if "downstream_proxy_port" in params and not isinstance(
            params["downstream_proxy_port"], int
        ):
            raise InvalidParameterError(
                "Bad parameter: downstream_proxy_port must be an int"
            )
        if "agent_platform" in params and not isinstance(
            params["agent_platform"], str
        ):
            raise InvalidParameterError(
                "Bad parameter: agent_platform must be an str"
            )
        ...
        response, _options = Api.send_request(
            "POST",
            "/remote_servers/{id}/authenticate_agent".format(
                id=quote(str(params["id"]), safe="")
            ),
            params,
            self.options,
...
def find_ping_agent(id, params=None, options=None):
    if not isinstance(params, dict):
        params = {}
    if not isinstance(options, dict):
        options = {}
    ...
    response, options = Api.send_request(
        "GET",
        "/remote_servers/{id}/ping_agent".format(
            id=quote(str(params["id"]), safe="")
        ),
        params,
        options,
...
#   proxy_url - string - Files Agent Proxy URL
#   proxy_host_header - string - Files Agent Proxy Host Header
def disconnect_agent(id, params=None, options=None):
    if not isinstance(params, dict):
        params = {}
    if not isinstance(options, dict):
        options = {}
    ...
    if "id" in params and not isinstance(params["id"], int):
        raise InvalidParameterError("Bad parameter: id must be an int")
    if "proxy_url" in params and not isinstance(params["proxy_url"], str):
        raise InvalidParameterError("Bad parameter: proxy_url must be an str")
    if "proxy_host_header" in params and not isinstance(
        params["proxy_host_header"], str
    ):
        raise InvalidParameterError(
    ...
    return AgentV2Auth(response.data, options)
...
#   permission_set - string - Permission set
#   supports_push_updates - boolean - Agent supports push updates
def authenticate_agent(id, params=None, options=None):
    if not isinstance(params, dict):
        params = {}
    if not isinstance(options, dict):
        options = {}
    ...
    if "proxy_registration_refresh" in params and not isinstance(
        params["proxy_registration_refresh"], bool
    ):
        raise InvalidParameterError(
            "Bad parameter: proxy_registration_refresh must be an bool"
        )
    if "agent_version" in params and not isinstance(
        params["agent_version"], str
    ):
    ...
    if "supports_push_updates" in params and not isinstance(
        params["supports_push_updates"], bool
    ):
        raise InvalidParameterError(
            "Bad parameter: supports_push_updates must be an bool"
        )
    if "id" not in params:
        raise MissingParameterError("Parameter missing: id")
    response, options = Api.send_request(
...
def list_for_testing(params=None, options=None):
    ...
    if "files_agent_api_token" in params and not isinstance(
        params["files_agent_api_token"], str
    ):
        raise InvalidParameterError(
            "Bad parameter: files_agent_api_token must be an str"
        )
    if "files_agent_public_key" in params and not isinstance(
        params["files_agent_public_key"], str
    ):
        raise InvalidParameterError(
            "Bad parameter: files_agent_public_key must be an str"
        )
    if "files_agent_root" in params and not isinstance(
        params["files_agent_root"], str
    ):
    ...
    response, options = Api.send_request(
        "POST", "/remote_servers/list_for_testing", params, options
    )
