...
class DesktopConfigurationProfile:
    default_attributes = {
        "id": None,  # int64 - Desktop Configuration Profile ID
    ...
    def set_attributes(self, attributes):
        for (
            attribute,
            default_value,
        ) in DesktopConfigurationProfile.default_attributes.items():
    ...
    def get_attributes(self):
        attrs = {
            ...
            for k in DesktopConfigurationProfile.default_attributes
...
def find(id, params=None, options=None):
    ...
    return DesktopConfigurationProfile(response.data, options)
...
def create(params=None, options=None):
    ...
    if "name" in params and not isinstance(params["name"], str):
        raise InvalidParameterError("Bad parameter: name must be an str")
    if "mount_mappings" in params and not isinstance(
        params["mount_mappings"], dict
    ):
        raise InvalidParameterError(
            "Bad parameter: mount_mappings must be an dict"
        )
    if "workspace_id" in params and not isinstance(
        params["workspace_id"], int
    ):
    ...
    if "mount_mappings" not in params:
        raise MissingParameterError("Parameter missing: mount_mappings")
    response, options = Api.send_request(
        "POST", "/desktop_configuration_profiles", params, options
    )
    return DesktopConfigurationProfile(response.data, options)
...
def create_export(params=None, options=None):
    ...
    response, options = Api.send_request(
        "POST",
        "/desktop_configuration_profiles/create_export",
        params,
        options,
    )
...
def new(*args, **kwargs):
    return DesktopConfigurationProfile(*args, **kwargs)
